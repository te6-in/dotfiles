...
- **Don't re-run the command with `; echo "exit: $?"` (or similar) just to "verify."** The first run already told you everything. Re-running is pure waste — especially painful when the command is slow (`tsc` on a large project, full `bun packages:build`, etc.).
- **If you genuinely want an explicit success marker baked into the first run**, append it at invocation time: `tsc --noEmit && echo ok`. Pay the cost once, not twice.
