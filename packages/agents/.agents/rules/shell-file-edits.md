...
4. **Fall back to a structured read-then-edit if any step is uncertain.** The structured tools don't misinterpret regex escapes or newline handling, and the surface area for silent corruption is much smaller. "It probably works" is not a good reason to run `sed -i`.
